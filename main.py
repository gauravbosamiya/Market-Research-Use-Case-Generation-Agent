...
#     # Resource Assets Section for PDF
#     if "resource_assets" in result_data:
#         story.append(Paragraph("Resource Assets", heading_style))
#         ra = result_data["resource_assets"]
        
#         if hasattr(ra, 'use_case_resources') and ra.use_case_resources:
#             for res in ra.use_case_resources:
#                 story.append(Paragraph(f"{res.use_case_title} ({getattr(res, 'category', 'Unknown')})", subheading_style))
#                 story.append(Paragraph(f"<b>Technology Focus:</b> {getattr(res, 'technology_focus', 'Not specified')}", normal_style))
                
#                 # Handle different resource data structures
#                 resource_types = [
#                     ('kaggle_datasets', 'Kaggle Datasets'),
#                     ('huggingface_resources', 'HuggingFace Resources'),
#                     ('github_repositories', 'GitHub Repositories'),
#                     ('additional_resources', 'Additional Resources')
#                 ]
                
#                 for attr_name, display_name in resource_types:
#                     if hasattr(res, attr_name):
#                         resources = getattr(res, attr_name)
#                         if resources:
...
#                                     story.append(Paragraph(f"• {title}: {resource.url}", bullet_style))
#                                 elif hasattr(resource, 'title') and hasattr(resource, 'url'):
#                                     story.append(Paragraph(f"• {resource.title}: {resource.url}", bullet_style))
                
#                 story.append(Spacer(1, 0.15*inch))
#         else:
#             story.append(Paragraph("No resource assets available.", normal_style))
...
#     """Flexible resource display that handles different data structures"""
#     if not resources:
...
#             # Dictionary with url and possibly title/description
...
#     # Resource Assets Section - Enhanced and Flexible
#     if "resource_assets" in result:
#         st.subheader("Resource Assets")
#         ra = result["resource_assets"]

#         # Display platform summary if available
...
#                             # Check if it's the raw object representation
#                             if ref.startswith("title="):
#                                 # Parse the object string representation
#                                 try:
...
#                                     st.markdown(f"- {ref}")
#                             else:
#                                 # Regular string reference
#                                 if ref.startswith('http'):
#                                     st.markdown(f"- [{ref}]({ref})")
#                                 else:
#                                     st.markdown(f"- {ref}")
#                         elif hasattr(ref, 'url') and hasattr(ref, 'title'):
#                             # Object with url and title attributes
...
if st.session_state.result:
    ...
    if "resource_assets" in result:
        ...
        # if hasattr(ra, 'use_case_resources') and ra.use_case_resources:
        #     for res in ra.use_case_resources:
        #         title = getattr(res, 'use_case_title', 'Unknown Use Case')
        #         category = getattr(res, 'category', 'Unknown Category')
                
        #         with st.expander(f"{title} ({category})"):
        #             # Technology focus
        #             tech_focus = getattr(res, 'technology_focus', 'Not specified')
        #             st.markdown(f"**Technology Focus:** {tech_focus}")
        ...
        #             for attr_name, display_name in resource_mappings:
        #                 if hasattr(res, attr_name):
        #                     resources = getattr(res, attr_name)
        #                     display_resources_flexible(resources, display_name)
        #                 else:
        #                     st.info(f"No {display_name} available")
        # else:
        #     st.warning("No resource assets were generated")
